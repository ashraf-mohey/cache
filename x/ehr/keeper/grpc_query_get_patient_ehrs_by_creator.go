...
func (k Keeper) GetPatientEhrsByCreator(goCtx context.Context, req *types.QueryGetPatientEhrsByCreatorRequest) (*types.QueryGetPatientEhrsByCreatorResponse, error) {
	if req == nil {
	...
	pageRes, err := query.Paginate(ehrsStore, req.Pagination, func(key []byte, value []byte) error {
		...
		if ehr.PatientId == req.PatientId && ehr.Creator == req.Creator {
	...
	return &types.QueryGetPatientEhrsByCreatorResponse{Ehrs: ehrs, Pagination: pageRes}, nil
}
